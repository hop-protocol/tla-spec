...
\begin{document}
\tlatex
\@x{}\moduleLeftDash\@xx{ {\MODULE} HopProtocol}\moduleRightDash\@xx{}%
\@x{ {\EXTENDS} Integers ,\, Naturals ,\, TLC ,\, Sequences ,\, FiniteSets}%
\@pvspace{8.0pt}%
\@x{ {\CONSTANT}}%
\@x{\@s{8.2} B ,\,\@s{21.96}}%
\@y{\@s{0}%
...
\@x{\@s{8.2} {\IF} S \.{=} \{ \} \.{\THEN} value}%
\@x{\@s{8.2} \.{\ELSE} \.{\LET} s \.{\defeq} Pick ( S )}%
\@x{\@s{39.51} \.{\IN} {\IF} Op ( s ,\, value ) \.{=} Op ( value ,\, s )}%
 \@x{\@s{39.51} \.{\THEN} SetReduce ( Op ,\, S \.{\,\backslash\,} \{ s \} ,\,
 Op ( s ,\, value ) )}%
\@x{\@s{39.51} \.{\ELSE} Assert ( {\FALSE} ,\,\@w{Err} )}%
\@pvspace{8.0pt}%
\@x{ SumSeq ( S ) \.{\defeq}}%
\@x{\@s{8.2} \.{\LET}\@s{5.76} seq\@s{0.93} \.{\defeq} S}%
 \@x{\@s{34.36} Sum [ i \.{\in} 1 \.{\dotdot} Len ( seq ) ] \.{\defeq} {\IF} i
 \.{=} 1 \.{\THEN} seq [ i ] \.{\ELSE} seq [ i ] \.{+} Sum [ i \.{-} 1 ]}%
 \@x{\@s{8.2} \.{\IN} {\IF} seq \.{=} {\langle} {\rangle} \.{\THEN} 0
 \.{\ELSE} Sum [ Len ( seq ) ]}%
\@pvspace{8.0pt}%
\@x{ {\RECURSIVE} SeqFromSet ( \_ )}%
\@x{ SeqFromSet ( S ) \.{\defeq}}%
\@x{\@s{8.2} {\IF} S \.{=} \{ \} \.{\THEN} {\langle} {\rangle}}%
 \@x{\@s{8.2} \.{\ELSE} \.{\LET} x \.{\defeq} {\CHOOSE} x \.{\in} S \.{:}
 {\TRUE}}%
 \@x{\@s{39.51} \.{\IN} {\langle} x {\rangle} \.{\circ} SeqFromSet ( S
 \.{\,\backslash\,} \{ x \} )}%
\@pvspace{16.0pt}%
\@x{ Hash ( v ) \.{\defeq} {\CHOOSE} n \.{\in} \{ Int \} \.{:} {\TRUE}}%
\@pvspace{8.0pt}%
 \@x{ {\ASSUME} ( Hash ( {\langle} 1 {\rangle} ) \.{=} Hash ( {\langle} 1
 {\rangle} ) )}%
 \@x{ {\ASSUME} ( Hash ( {\langle} \{ 1 ,\, 2 ,\, 3 \} {\rangle} ) \.{=} Hash
 ( {\langle} \{ 2 ,\, 1 ,\, 3 ,\, 1 \} {\rangle} ) )}%
\@pvspace{8.0pt}%
\@x{ TypeOK \.{\defeq}}%
\@x{\@s{8.2} \.{\land} B \.{\subseteq} 1 \.{\dotdot} 5}%
 \@x{\@s{8.2} \.{\land} \E\, c \.{\in} 1 \.{\dotdot} 2 \.{:} C [ c ] . h
 \.{\in} 1 \.{\dotdot} 5}%
\@pvspace{8.0pt}%
\@x{ Init \.{\defeq}}%
\@x{\@s{8.2} \.{\land} C \.{=} [ c \.{\in} 1 \.{\dotdot} 2 \.{\mapsto} [}%
\@x{\@s{23.41} id\@s{4.13} \.{\mapsto} c ,\,\@s{65.6}}%
\@y{\@s{0}%
 chain \ensuremath{id
}}%
\@xx{}%
\@x{\@s{23.41} h\@s{7.46} \.{\mapsto} 1 ,\,\@s{65.76}}%
\@y{\@s{0}%
 block height
}%
\@xx{}%
\@x{\@s{23.41} pt\@s{3.96} \.{\mapsto} {\langle} {\rangle} ,\,\@s{62.98}}%
\@y{\@s{0}%
 pending transfers
}%
\@xx{}%
\@x{\@s{23.41} pts \.{\mapsto} 0 ,\,\@s{65.76}}%
\@y{\@s{0}%
 pending transfers sum
}%
\@xx{}%
\@x{\@s{23.41} tr\@s{4.72} \.{\mapsto} 0\@s{71.87}}%
\@y{\@s{0}%
 transfer root
}%
\@xx{}%
...
\@pvspace{8.0pt}%
\@x{ Add ( c ) \.{\defeq}}%
 \@x{\@s{8.2} \.{\LET} s\@s{1.42} \.{\defeq} {\langle} ( RandomElement ( 1
 \.{\dotdot} 2 ) ) {\rangle} \.{\circ} C [ c ] . pt \.{\IN} s}%
\@pvspace{8.0pt}%
\@x{ Send ( c ) \.{\defeq}}%
\@x{\@s{8.2} \.{\land} Print ( {\langle} C {\rangle} ,\, {\TRUE} )}%
\@x{\@s{8.2} \.{\land} C [ c ] . pts \.{<} 3}%
\@x{\@s{8.2} \.{\land} C \.{'} \.{=} [ C {\EXCEPT} {\bang} [ c ] \.{=} [}%
\@x{\@s{27.91} id\@s{4.13} \.{\mapsto} C [ c ] . id ,\,}%
\@x{\@s{27.91} h\@s{7.46} \.{\mapsto} C [ c ] . h ,\,}%
\@x{\@s{27.91} pt\@s{3.96} \.{\mapsto} Add ( c ) ,\,}%
\@x{\@s{27.91} pts \.{\mapsto} SumSeq ( Add ( c ) ) ,\,}%
\@x{\@s{27.91} tr\@s{4.72} \.{\mapsto} C [ c ] . tr}%
\@x{\@s{16.4} ] ]}%
\@pvspace{8.0pt}%
\@x{ Commit ( c ) \.{\defeq}}%
\@x{\@s{8.2} \.{\land} C [ c ] . pts \.{>} CT}%
\@x{\@s{8.2} \.{\land} C \.{'} \.{=} [ C {\EXCEPT} {\bang} [ c ] \.{=} [}%
\@x{\@s{27.91} id\@s{4.13} \.{\mapsto} C [ c ] . id ,\,}%
\@x{\@s{27.91} h\@s{7.46} \.{\mapsto} C [ c ] . h ,\,}%
\@x{\@s{27.91} pt\@s{3.96} \.{\mapsto} {\langle} {\rangle} ,\,}%
\@x{\@s{27.91} pts \.{\mapsto} 0 ,\,}%
\@x{\@s{27.91} tr\@s{4.72} \.{\mapsto} C [ c ] . pts}%
\@x{\@s{16.4} ] ]}%
\@pvspace{8.0pt}%
\@x{ Next \.{\defeq} \.{\land} \E\, bn \.{\in} B \.{:}}%
\@x{\@s{48.03} \.{\lor} \.{\land} \E\, c \.{\in} {\DOMAIN} C \.{:}}%
\@x{\@s{70.25} \.{\land} \.{\lor} Send ( C [ c ] . id )}%
\@x{\@s{81.36} \.{\lor} Commit ( C [ c ] . id )}%
 \@x{\@s{48.03} \.{\lor} \.{\land} C \.{'} \.{=} [ c \.{\in} {\DOMAIN} C
 \.{\mapsto} [}%
\@x{\@s{82.96} id\@s{4.13} \.{\mapsto} C [ c ] . id ,\,}%
\@x{\@s{82.96} h\@s{7.46} \.{\mapsto} bn ,\,}%
\@x{\@s{82.96} pt\@s{3.96} \.{\mapsto} C [ c ] . pt ,\,}%
\@x{\@s{82.96} pts \.{\mapsto} C [ c ] . pts ,\,}%
\@x{\@s{82.96} tr\@s{4.72} \.{\mapsto} C [ c ] . tr}%
\@x{\@s{70.25} ] ]}%
\@pvspace{8.0pt}%
 \@x{ Spec\@s{1.46} \.{\defeq} Init\@s{2.49} \.{\land} {\Box} [ Next ]_{
 {\langle} C {\rangle}}}%
\@x{}\bottombar\@xx{}%
\end{document}
